    ...
    switch(state) {
        ...
        case VCSessionStateError:
            ...
            if(!isReconnectStatus)
            {
                ...
                //start reconnect
                [self performSelectorOnMainThread:@selector(startReConnectTimer:) withObject:nil waitUntilDone:NO];
            }
            break;
        default:
            break;
...
- (void)doCheckOnce:(NSTimer*)theTimer
{
    if([_broadcasterView.status.text isEqualToString:@"Couldn't connect to server"] || [_broadcasterView.status.text isEqualToString:@"Disconnected"])
    {
        //do reconnect
        [_session startRtmpSessionWithURL:self.publishUrl andStreamKey:self.publishStreamName];
    }
    
    if ([_broadcasterView.status.text isEqualToString:@"Connecting to server ..."]) {
        //release
        [_session endRtmpSession];
        //do reconnect
        [_session startRtmpSessionWithURL:self.publishUrl andStreamKey:self.publishStreamName];
...
- (void)startReConnectTimer:(CineBroadcasterViewController*)the
{
    if(timer!=nil)
    {
        [timer invalidate];
    ...
    timer =  [NSTimer scheduledTimerWithTimeInterval:10.0 target:self selector:@selector(doCheckOnce:) userInfo:nil repeats:YES];
}

- (void)endReConnectTimer:(CineBroadcasterViewController*)the
{
    if(timer!=nil)
    {
        [timer invalidate];
